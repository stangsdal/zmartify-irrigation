...
static void test_running_zone_transition_keeps_master_open(void)
{
    irrigation_engine_t engine;
    call_count = 0;
    irrigation_engine_init(&engine);
    ...
    assert(calls[3].call == CALL_ZONE_OPEN && calls[3].relay == 2);
    assert(engine.active_zone_id == 2);
    assert(engine.active_relay_index == 2);
    assert(irrigation_engine_remaining_seconds(&engine, 5000) == 45);
...
int main(void)
{
    ...
    test_running_zone_transition_keeps_master_open();
